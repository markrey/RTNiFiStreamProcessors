...
@Tags({"RTMQTTAudioProcessor"})
@CapabilityDescription("Processes RTMQTT audio messages")
@SeeAlso({})
@ReadsAttributes({@ReadsAttribute(attribute="", description="")})
@WritesAttributes({@WritesAttribute(attribute="Filename", description="Sets filename to the correct cycled file")})
public class RTMQTTAudioProcessor extends AbstractProcessor {
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        try {
            bytes = Base64.getDecoder().decode(obj.remove("audio").toString());
        } catch(Exception e) {
            getLogger().error("Audio was not in correct base64 format");
            return;
        }
